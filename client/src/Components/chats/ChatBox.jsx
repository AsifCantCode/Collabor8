...
const ChatBox = ({ chatLoading, setLoadingChat }) => {
  ...
  useEffect(() => {
    socket.on("message recieved", (newMessageRecieved) => {
      ...
      if (
        ...
        if (!chats.find((chat) => chat._id === newMessageRecieved.chat._id))
  ...
  const sendMessage = async (acceptOrReject) => {
    ...
    let messageType;
    console.log(!selectedChat?.latestMessage, selectedChat?.isOpen === "close");
    ...
    <div className={`${classes["chat-box"]}`}>
      ...
      {selectedChat && (
        <div className={`${classes["chat-header"]}`}>
          ...
          >
            {selectedChat?.users?.find((user) => user?._id !== newUser._id)
              .image !== undefined ? (
              <img
          ...
          <>
            <ScrollableFeed>
              {messages?.map((message) => (
                <div className={`${classes["message"]}`} key={message?._id}>
                  <div
                    className={`${
                      message?.sender?._id === newUser?._id
                  ...
                  {message?.messageType !== "normal" && (
                    ...
                    <div className={`${classes["special-message"]}`}>
                      {message?.messageType === "request" &&
                        selectedChat?.isOpen === "pending" && (
                          <>
                            <p>Request Pending...</p>
                            {newUser?._id !== message?.sender?._id && (
                              <>
                                <SmallButtonAc
                                  func={(e) => {
                                    e.preventDefault();
        ...
        <div className={`${classes["message-input"]}`}>
          ...
          {selectedChat?.isOpen !== "pending" && (
            <>
              ...
              <Button
                text={selectedChat?.isOpen === "open" ? "Send" : "Request"}
                func={sendMessage}
              />
            </>
          )}
