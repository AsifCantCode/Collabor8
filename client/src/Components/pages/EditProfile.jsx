...
const EditProfile = () => {
  ...
  useEffect(() => {
    if (newUser) {
      ...
      console.log("Previous Image: ", typeof newUser?.image);
    }
  }, [newUser]);
  ...
  return (
    <>
      ...
      <div className={`${classes["EditProfile"]}`}>
        <div className={`${classes["page-header"]} global-single-page-header`}>
          <Link to={`/`}>
            <FaLongArrowAltLeft />
            Go to Home
          </Link>
        ...
        <div className={`${classes["edit-profile-area"]}`}>
          ...
          >
            <div className={`${classes["image"]}`}>
              ...
              {typeof previousImage === "object" && (
                <div className={classes.imageBox}>
                  <img src={makeProfileImageURL(previousImage)} alt="" />
                  <span className={`${classes["delete-btn"]}`}>
                    <RiDeleteBin5Fill
                      onClick={() => handleImageDelete("previous")}
                    />
                  </span>
                </div>
              )}
              {selectedImage && (
                <div className={classes.imageBox}>
                  ...
                  <span className={`${classes["delete-btn"]}`}>
                    <RiDeleteBin5Fill
                      onClick={() => handleImageDelete("selected")}
                    />
                  </span>
                </div>
              )}
            ...
            <div>
              ...
              />
              <span className={bio ? `${classes["valid"]}` : ""}>Bio</span>
            </div>
              ...
              <div className={`${classes["tag-list"]}`}>
                {filteredTag?.length > 0 &&
                  filteredTag.map((tag, index) => (
                    <span
                      className={`${
                        checkSelected(tag) ? classes["selectedTag"] : ""
                      }`}
                      onClick={() => {
                        handleTagChange(tag);
                      }}
